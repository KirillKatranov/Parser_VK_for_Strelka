...
class ParsingVK(BaseGateway):
    ...
    def parsing(self, amount: int = None) -> None:
        ...
        self.list_response = []
        amount_group_for_parsing = len(self.list_groups_id)
        if self.amount_posts > amount_group_for_parsing:
            self.amount_posts_from_single_group = self.amount_posts // amount_group_for_parsing
